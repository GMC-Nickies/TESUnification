...
MWfull_path = MWfilepath + 'PlayerDataMW.json'
OBfull_path = OBfilepath + 'PlayerDataMW.json'
...
with open(OBfull_path) as json_file:
	...
	while counter1 < size:
		...
		##print("Last effect #: " + str(lastEffect))
		if data[baseString + "Type"] == 2:
			data[baseString + "Cost"] = 0
		...
		while(counter <= lastEffect):
			...
			if data[effectIDString] == 'NONE':
				...
				data[effectAttString] = 7
				effectMagString = baseString2 + str(counter) + "magnitude"
				data[effectMagString] = 0
				effectDurString = baseString2 + str(counter) + "duration"
				data[effectDurString] = 0
	...
	data["Blade"] = max(data["Longblade"], data["Shortblade"])
	data["Blunt"] = max(data["Bluntweapon"], data["Axe"])
